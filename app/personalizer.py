...
def _generate_llm_points(
    ...
    prompt: str = (
        f"You are helping a high school student write ONE genuine, specific "
        f"cold email to a professor about research. Produce talking points the "
        f"student can honestly use, grounded ONLY in the professor's research "
        f"below.\n\n"
        f"Hard rules:\n"
        f"- Never fabricate papers, results, dates, or details not given below.\n"
        f"- No generic flattery ('I admire your impressive/groundbreaking "
        f"work'). Specific beats impressive.\n"
        f"- Each point must reference something CONCRETE about the professor's "
        f"actual research, and connect to the student's interests only where "
        f"there is a real overlap.\n"
        f"- Voice: a curious, humble high schooler who has done some reading — "
        f"not an expert. No overclaiming.\n\n"
        f"PROFESSOR\n"
        f"Name: {prof.name}\n"
        f"Field: {prof.field}\n"
        f"Research focus: {prof.summary or prof.research_summary or 'N/A'}\n"
        f"Key topics: {', '.join(prof.keywords_list) or 'N/A'}\n"
        f"Recent work: {prof.recent_work or 'N/A'}\n\n"
        f"STUDENT\n"
        f"Interests: {sender.interests or 'N/A'}\n"
        f"Background: {sender.background or 'N/A'}\n\n"
        f"Write exactly 3 talking points, each 1-2 sentences.\n"
        f"Return ONLY JSON: {{\"talking_points\": [\"point1\", \"point2\", \"point3\"]}}"
    )
